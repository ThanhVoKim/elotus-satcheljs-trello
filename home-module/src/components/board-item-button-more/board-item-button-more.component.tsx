...
import { BoardItemEditDialog } from 'src/components/board-item-edit-dialog';
import { Modal } from 'src/components/modal';
import boardsStore from 'src/store-board';
import { MoreVerticalIcon } from 'src/svg';
...
export const BoardItemButtonMore: React.FC<IBoardItemButtonMoreProps> = (
	...
	const handleDeleteBoard = () => {
		boardsStore.deleteBoardAction(board.id!);
	};

	const handleUpdateBoard = (title: string) => {
		boardsStore.editBoardAction(board.id!, title);
		handleCloseDialog();
	};
